...
RESULTADO_ACTUAL: |
  `pronto_guard.sh` y `pronto-smoke-api-isolation` no resolvían correctamente rutas del repo o validaban templates legacy inexistentes.
...
SOLUCION: |
  Se normalizaron rutas dinámicas (`REPO_ROOT`/`SCRIPTS_ROOT`) en `pronto_guard.sh` y se actualizó `pronto-smoke-api-isolation` para SPA: uso de `index.html`, wrapper HTTP canónico real (`pronto-static/src/vue/employees/shared/core/http.ts`), eliminación de paths absolutos locales, corrección de incrementos bajo `set -e` y de capturas HTTP para no abortar por 401 esperados.
