...
ESTADO: RESUELTO
SOLUCION: `pronto-scripts/bin/test-all.sh` fue reemplazado por un wrapper deprecado que delega a `pronto-tests/scripts/run-tests.sh functionality`, eliminando la ejecución del suite bash legacy (`test-api.sh`, `test-tips-flow.sh`, `test-anonymous.sh`) incompatible con CSRF y autenticación canónica.
